    ...
    <div className={`w-screen ${open.open && !open.isEdit ? "" : "hidden"}  flex items-center justify-center fixed flex-col left-0 right-0 top-0 h-screen bg-gray-900/90 p-2 md:p-3`}>
      ...
      <div className='w-full h-full scrollbar-thin scrollbar-thumb-cyan-600 overflow-y-auto bg-black/60 p-2 md:p-5 lg:w-4/5 md:w-3/4'>
        <div className='flex py-10 justify-center bg-transparent '>
          <input disabled={open.isEdit} value={mydata?.contact} onChange={(e) => dispatch(setUserContact(e.target.value))} ref={p} type="text" className={`${isUpdate ? "bg-gray-500" : "bg-transparent"} rounded-md border  border-gray-700 py-2 px-10 text-center text-white`} placeholder='WhatsApp No(Visible only in mail)' />
        </div>
        ...
        <div className='grid grid-cols-4 lg:grid-cols-8 p-2 mt-3 gap-1 grid-flow-row'>
          {section.map((val, index) => {
            return <button key={index} onClick={() => dispatch(setAllotedSection(val))} className={`bg-gray-900 ${ mydata?.alloted === val && "bg-green-700"}  rounded-md  p-2`}>{val}</button>
          })}
        </div>
        ...
        <div className='grid grid-cols-4 p-2 mt-3 lg:grid-cols-8 gap-1 grid-flow-row'>
          {section.map((val, index) => {
            return <button disabled={mydata?.alloted == val} key={index} onClick={() => dispatch(pushLookingFor(val))} className={`bg-gray-900 disabled:bg-gray-600 ${mydata?.lookingFor.includes(val) ? "bg-green-600" : ""}  rounded-md p-2`}>{val}</button>
          })}
        </div>
