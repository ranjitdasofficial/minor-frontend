...
import {
  ...
  addSolutionToPyqs,
  updatePyqsQuestion,
} from "@/ServerActions/Notes/Notes";
...
export function UpdatePYQSQuestion() {
  ...
  const data = useAppSelector(
    (state) => state.AcademicSlice.updatePYQSQuestion
  );
  ...
  async function onSubmit() {
    ...
    const res = await updatePyqsQuestion(data.subjectId, data.pyqId, fileId,data.Type);
    if (res.status !== 201) {
            ...
            <div>
              <Link
                ...
                href={`/academic/view/Kiitconnect-Ru6F3${1}${fileId}?secure=${
                  session?.data?.authToken ? session.data?.authToken : secureId
                }&sid=${1}`}
              >
                Open Link
              </Link>
